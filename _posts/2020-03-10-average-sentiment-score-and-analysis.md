---
layout: post
title: Average Sentiment Score and Analysis
subtitle: Getting Effective Sentiment
...
In this part, we use all of three packages: TextBlob, Vader and Affin. We calculate **Average Sentiment Score** and carry out **Sentiment Analysis**. As our dataset contains tweet, reddit, AFFIN Sentiment score, TextBlob Sentiment score and VADER sentiment score. 

- For **AFFIN**, if the AFFIN score is positive we have given it as +1 and for negative it is -1 and zero remain as 0.
- For **TextBlob**, we have compared the positive and negative sentiment scores if positive sentiment is greater than negative sentiment we have given it a +1 and -1 for the vice versa, if both are equal then it is given a 0.
- For **VADER** we followed the same technique of TextBlob. 

We also check the sum of all sentiment scores of 3 classifiers. If it is greater than or equal to 1, then the effective polarity is given 1; if it is less than or equal to -1 then the tweet is given an effective polarity of -1. For all other cases, it is given a 0. 
...
cleaned_tweets = pd.read_csv('/Users/mayiwei/Desktop/Sentiment Polarity Regression/cleaned_tweets.csv')
...
def sentiments(text):
    words = pattern_split.split(str(text).lower())
    sentiment = 0
    for i in words:
        if i in afinn_data:
...
text = cleaned_tweets.loc[4,"tweet"]
words = pattern_split.split(str(text).lower())
sentiment = 0
for i in words:
    if i in afinn_data:
...
cleaned_tweets["Affin_score"]  = cleaned_tweets["tweet"].apply(sentiments)
...
Here is our code:
```javascript
cleaned_tweets["datem"] = pd.to_datetime(cleaned_tweets["date"])
cleaned_tweets.set_index(cleaned_tweets["datem"],inplace=True)
...
cleaned_tweets["Effective_Affin"] = cleaned_tweets["Affin_score"].apply(eas)
cleaned_tweets["Effective_TextBlob"] = cleaned_tweets["Text_Blob_polarity"].apply(eas)

# filtering only posive rows neglecting negative and neutral ones
x = cleaned_tweets["Vader_Pos"] > cleaned_tweets["Vader_Neg"]

# filtering only negative rows neglecting positive and neutral ones
y = cleaned_tweets["Vader_Pos"] < cleaned_tweets["Vader_Neg"]

# intialize all with zeros and neutral tweets gets automatically zero
cleaned_tweets["Effective_Vader"] = 0 

cleaned_tweets.loc[x,"Effective_Vader"] = 1
cleaned_tweets.loc[y,"Effective_Vader"] = -1

Sentiment_Scores = cleaned_tweets.loc[:,["dt","tweet", "Effective_Affin", "Effective_TextBlob", "Effective_Vader"]]

Sentiment_Scores['all values'] = Sentiment_Scores[['Effective_Affin',"Effective_TextBlob", "Effective_Vader"]].values.tolist()
...
Sentiment_Scores["effective_polarity"] = Sentiment_Scores['all values'].apply(cal)

Sentiment_Scores.to_csv("effective_polarity.csv")

df = Sentiment_Scores[["dt","tweet","effective_polarity"]]

df = df.resample('D').mean()

plt.plot(df["effective_polarity"])
...
for ret in ['ret_next','ret_next2','ret_next3','ret_next4','ret_next5','ret2','ret3','ret4','ret5']:
    model = smf.ols('%s ~ effective_polarity'%ret, 
                    data=final[
        (final.Date>start_date) & (final.Date<end_date)]).fit()
    beta = model.params[1]
    t = model.tvalues[1]
    p = model.pvalues[1]
    r = model.rsquared
...
smf.ols('ret_next ~ effective_polarity', data=final).fit().summary()

fig,ax = plt.subplots(figsize=(14,8))
ax1 = ax.twinx()
final.set_index('Date')['effective_polarity'].plot(ax=ax, label='final(Left)',color='r')
final.set_index('Date')['ret_next4'].plot(ax=ax1, label='BTC Daily Return 4 Days Later(Right)',color='b')
ax.set_ylabel('effective_polarity')
ax1.set_ylabel('Daily Ret')
lines, labels = ax.get_legend_handles_labels()
lines2, labels2 = ax1.get_legend_handles_labels()
ax1.legend(lines + lines2, labels + labels2, loc=0)
plt.savefig('effective_polarity.jpg',dpi=600)
